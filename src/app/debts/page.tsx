...
export default function DebtsPage() {
    ...
    // Form state - Form is now permanently visible
    ...
    const [settleDebtIds, setSettleDebtIds] = useState<string[] | null>(null);
    ...
    const handleAddDebt = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                setNote('');
                fetchDebts();
                // We keep personName for potentially adding more entries for same person
            } else {
    ...
    const openSettleModal = (debt: Debt) => {
        if (debt.type === 'credit') {
            ...
            setSettleDebtIds(null);
            setSettleDebtItem(debt);
            setExpenseCategory('Other');
        }
    };

    const openSettleAllModal = (personName: string, personDebts: Debt[]) => {
        const creditTotal = personDebts.filter(d => d.type === 'credit').reduce((sum, d) => sum + Number(d.amount), 0);
        const debitTotal = personDebts.filter(d => d.type === 'debit').reduce((sum, d) => sum + Number(d.amount), 0);
        const net = creditTotal - debitTotal;

        if (net >= 0) {
            // If they owe us net (or net 0), just settle everything
            const ids = personDebts.map(d => d.id);
            executeBatchSettle(ids);
        } else {
            // If we owe them net, we should ask about logging expense
            const ids = personDebts.map(d => d.id);
            setSettleDebtIds(ids);
    ...
    const executeBatchSettle = async (ids: string[]) => {
        setSettling(true);
        try {
            const res = await fetch('/api/debts', {
                method: 'PATCH',
            ...
            if (res.ok) {
                showToast('Batch marked as settled');
                fetchDebts();
            }
        } catch {
            showToast('Failed to settle', 'error');
    ...
    const confirmSettleWithExpense = async (logAsExpense: boolean) => {
        if ((!settleDebtId && !settleDebtIds) || !settleDebtItem) return;
            ...
            try {
                ...
                await fetch('/api/expenses', {
                    ...
                    body: JSON.stringify({
                        ...
                        description: `Settled Debt: ${settleDebtItem.person_name} ${settleDebtIds ? '(Batch)' : ''}`,
        ...
        if (settleDebtIds) {
            await executeBatchSettle(settleDebtIds);
        } else {
    ...
    const handleDelete = async (id: string | string[]) => {
        ...
        const confirmMsg = isBatch ? 'Are you sure you want to delete all records for this person?' : 'Remove this record?';
        if (!confirm(confirmMsg)) return;

        try {
            const url = isBatch ? `/api/debts?ids=${id.join(',')}` : `/api/debts?id=${id}`;
            const res = await fetch(url, { method: 'DELETE' });
            if (res.ok) {
                showToast(isBatch ? 'All records removed' : 'Debt removed');
                fetchDebts();
            }
        } catch {
        ...
        <div className="fade-in">
            ...
            {settleDebtItem && (
                <div className="card fade-in" style={{ marginBottom: 'var(--space-lg)', border: '1px solid var(--primary)' }}>
                    <h3 className="section-title" style={{ marginBottom: 'var(--space-sm)' }}>Settle Debit {settleDebtIds ? '(Batch)' : ''}</h3>
                    <p style={{ color: 'var(--text-secondary)', marginBottom: 'var(--space-md)', fontSize: '14px' }}>
                        You are settling <strong>{formatCurrencyPlain(Number(settleDebtItem.amount))}</strong> to {settleDebtItem.person_name}.
                        Would you like to log this as a daily expense?
                    </p>
                    ...
                    <div style={{ display: 'flex', gap: 'var(--space-md)' }}>
                        <button className="btn btn-primary" onClick={() => confirmSettleWithExpense(true)} disabled={settling}>
                            {settling ? 'Processing...' : 'Yes, Log as Expense'}
                        </button>
                        <button className="btn" onClick={() => confirmSettleWithExpense(false)} disabled={settling}>
                            No, Just Settle It
                        </button>
                        <button className="btn btn-danger" onClick={() => { setSettleDebtId(null); setSettleDebtIds(null); setSettleDebtItem(null); }} disabled={settling}>
                            Cancel
                        </button>
                    </div>
                </div>
            ...
            {loading ? (
                        ...
                        <div key={name} className="list-item" style={{ flexDirection: 'column', alignItems: 'stretch' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%', marginBottom: 'var(--space-sm)' }}>
                                <div className="list-item-content">
                                    <div className="list-item-icon" style={{ background: data.net === 0 ? 'var(--neutral-bg)' : data.net > 0 ? 'var(--success-bg)' : 'var(--danger-bg)' }}>
                                        {data.net === 0 ? '⚖️' : data.net > 0 ? '↙️' : '↗️'}
                                    </div>
                                    <div className="list-item-text">
                                        <span className="list-item-title" style={{ textTransform: 'capitalize' }}>{name}</span>
                                        <span className="list-item-subtitle">{data.debts.length} active {data.debts.length === 1 ? 'record' : 'records'}</span>
                                    </div>
                                </div>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-md)' }}>
                                    <span className="list-item-amount" style={{ color: data.net === 0 ? 'var(--text-secondary)' : data.net > 0 ? 'var(--success)' : 'var(--danger)' }}>
                                        {data.net === 0 ? 'Settled (Net 0)' : `${data.net > 0 ? '+' : ''}${formatCurrencyPlain(data.net)}`}
                                    </span>
                                    <div className="list-item-actions">
                                        <button className="btn btn-success btn-sm" onClick={() => openSettleAllModal(name, data.debts)} title="Settle All">
                                            ✓ All
                                        </button>
                                        <button className="btn btn-danger btn-sm" onClick={() => handleDelete(data.debts.map(d => d.id))} title="Delete All">
                                            ✕ All
                                        </button>
                                    </div>
                                </div>
                                    ...
                                    <div key={debt.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: 'var(--space-xs) 0', borderTop: '1px solid var(--border)' }}>
                                        {editingId === debt.id ? (
                                            <div className="edit-form" style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-sm)', width: '100%' }}>
                                                <div className="form-row">
                                                    <input type="text" className="form-input" value={editPersonName} onChange={(e) => setEditPersonName(e.target.value)} required />
                                                    <input type="number" className="form-input" value={editAmount} onChange={(e) => setEditAmount(e.target.value)} required />
                                                </div>
                                                <div className="form-row">
                                                    <select className="form-select" value={editType} onChange={(e) => setEditType(e.target.value as 'credit' | 'debit')}>
                                                        <option value="credit">Credit</option>
                                                        <option value="debit">Debit</option>
                                                    </select>
                                                    <input type="date" className="form-input" value={editEntryDate} onChange={(e) => setEditEntryDate(e.target.value)} required />
                                                </div>
                                                <input type="text" className="form-input" value={editNote} onChange={(e) => setEditNote(e.target.value)} placeholder="Note" />
                                                <div style={{ display: 'flex', gap: 'var(--space-sm)', justifyContent: 'flex-end' }}>
                                                    <button className="btn btn-sm" onClick={handleCancelEdit}>Cancel</button>
                                                    <button className="btn btn-primary btn-sm" onClick={handleSaveEdit} disabled={savingEdit}>{savingEdit ? 'Saving...' : 'Save'}</button>
                                                </div>
                                            </div>
                                        ) : (
                                            <>
                                                ...
                                                <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-sm)' }}>
                                                    ...
                                                    <div className="list-item-actions">
                                                        <button className="btn btn-sm" onClick={() => handleEditClick(debt)} style={{ padding: '4px' }}>✏️</button>
                                                        <button className="btn btn-success btn-sm" onClick={() => openSettleModal(debt)} style={{ padding: '4px' }}>✓</button>
                                                        <button className="btn btn-danger btn-sm" onClick={() => handleDelete(debt.id)} style={{ padding: '4px' }}>✕</button>
                                                    </div>
                                                </div>
                                            </>
                                        )}
                ...
                <div className="list">
                    {filteredDebts.map((debt) => (
                        <div key={debt.id} className="list-item">
                            <div className="list-item-content">
                                <div className="list-item-icon" style={{ background: 'var(--neutral-bg)' }}>⚖️</div>
                                <div className="list-item-text">
                                    <span className="list-item-title" style={{ textTransform: 'capitalize' }}>{debt.person_name}</span>
                                    <span className="list-item-subtitle">
                                        {debt.note || 'Settled record'} · {new Date(debt.entry_date).toLocaleDateString()}
                                    </span>
                                </div>
                            </div>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-md)' }}>
                                <span className="list-item-amount" style={{ color: 'var(--text-secondary)' }}>{formatCurrencyPlain(Number(debt.amount))}</span>
                                <button className="btn btn-danger btn-sm" onClick={() => handleDelete(debt.id)}>✕</button>
                            </div>
                        </div>
                    ))}
                </div>
                ...
                <div className="empty-state">
                    ...
                    <p className="empty-state-text">No {activeTab} debts.</p>
                </div>
            )}
        </div>
    );
